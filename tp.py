...
for rr in range(0, len(liste_fichiers)):
    ...
    for l in range(0, int(len(all_lignes)/nb_event)):
         ...
         for col in [col1, col2, col3]:
             ...
             taille = len(col)                                            #####ATTENTION MODIF
         ...
         beta2 = (73.*10**(-2) )/(1.*c*(tt[2]-tt[1]))                             ###ATTENTION MODIF
         #beta3 = (10.5*10**(-2))/(1.*c*(tt[2]-tt[1]))
        
        
         #############Condition pour avoir des evenements coherents##########
         if beta2>0. and beta2<beta1 and beta1<1.:
             #########Calcul des gamma##########
             ...
             dE1 =P1*(0.5*np.log(abs((2*mec2*beta1**2*gamma1*T_max)/I**2))-beta1**2)
             print(dE1)
